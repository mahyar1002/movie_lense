...
def createCharacteristicMatrix(filename):
    ...
    data_frame = pd.read_csv(filename, sep=",", usecols=[0, 1, 2], names=['userID', 'movieID', 'rating'],
                             engine='python')
    data_mat = np.array(data_frame.pivot(index='userID', columns='movieID', values='rating').fillna(0))
    return data_mat.astype(int)
...
#             while (F(A_lazy_greedy.union(set([movie_index_sorted[0]]))) - F(A_lazy_greedy)) < \
#                     marginal_values_list_sorted[1]:
#                 # update the list of marginals
#                 marginal_values_list[movie_index_sorted[0]] = F(A_lazy_greedy.union(set([movie_index_sorted[0]]))) - F(
#                     A_lazy_greedy)
#
