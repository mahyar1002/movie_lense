...
class AutoEncoder(object):
    ...
    def __init__(self, data, validation_perc=0.2, lr=0.001,
                 intermediate_size=1000, encoded_size=100, cuda_enable=True):
        ...
        if self.cuda_enable:
            ...
            self.val = torch.from_numpy(self.val.values).type(torch.FloatTensor)
    ...
    def get_encoded_representations(self):
        if self.cuda_enable:
            to_encode = torch.from_numpy(self.data.values).type(torch.FloatTensor).cuda()
        else:
            to_encode = torch.from_numpy(self.data.values).type(torch.FloatTensor)
